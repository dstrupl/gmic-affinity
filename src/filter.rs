...
pub(crate) fn validate_filter_record(fr: &FilterRecord) -> Result<PixelBuffers, FilterError> {
    ...
    if width <= 0 || height <= 0 || width > MAX_EDGE || height > MAX_EDGE {
        return Err(FilterError::BadDimensions {
            width,
            height,
            planes,
        });
...
pub fn run_passthrough(fr: &mut FilterRecord) -> Result<(), FilterError> {
    ...
    for y in 0..buf.height as usize {
        ...
        let in_row = unsafe { std::slice::from_raw_parts(buf.in_data.add(in_start), row_pixels) };
        let out_row =
            unsafe { std::slice::from_raw_parts_mut(buf.out_data.add(out_start), row_pixels) };
...
mod tests {
    ...
    fn rejects_zero_dimensions() {
        unsafe {
            ...
            fr.out_row_bytes = 1;
            assert!(matches!(
                run_passthrough(&mut fr),
                Err(FilterError::BadDimensions { .. })
            ));
        }
    }
    ...
    fn rejects_absurd_dimensions() {
        unsafe {
            ...
            assert!(
                matches!(err, FilterError::BadDimensions { .. }),
                "got {err:?}"
            );
        }
    }
    ...
    fn rejects_null_in_data() {
        unsafe {
            ...
            assert!(matches!(
                run_passthrough(&mut fr),
                Err(FilterError::NullPointer("in_data"))
            ));
        }
    }
    ...
    fn rejects_small_row_stride() {
        unsafe {
            ...
            assert!(matches!(
                run_passthrough(&mut fr),
                Err(FilterError::RowStrideTooSmall { .. })
            ));
        }
    }
}
